...
2. **Read stack information:**
   - Call `read_memory('stack')` to get tech stack choices
   ...
   - If no stack memory exists, skip stack-specific checks (continue with generic checks only)
...
These heuristics are applied when `.serena/memories/stack` exists:
...
- **Stack-aware checks**: Only run when `.serena/memories/stack` exists
